...
So, we need to wait a little bit, and **importantly** we need to call **tick**/**poll** on the device so that it updates its awaiting tasks. This is a part of the API that is **not standard yet**, so we must adapt our implementation to the backend:
...
To avoid using an arbitrary number of ticks, we may set a **context boolean** to `true` in `onQueueWorkDone` and break the loop as soon as it is true. But soon we will call this in the main application loop anyway!
