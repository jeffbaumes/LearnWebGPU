...
We can now submit a **buffer-buffer copy** operation to the command queue. This is not directly available from the queue object but rather requires us to **create a command encoder**.
We will use a command encoding process similar to the render pass one:
...
We then simply add the following to perform the buffer-buffer copy:
